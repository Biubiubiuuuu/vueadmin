  ...
  <div>
    <div class="dashboard-container">
      <el-radio-group v-model="status" @change="statusChange">
        <el-radio-button v-for="(item, index) in stateBtns" :key="index" :label="item.recState">
          <i :class="item.class" />
          {{ item.recStateName }}({{ item.count }})
        </el-radio-button>
      </el-radio-group>
    </div>
          ...
          <el-collapse v-for="(item,index) in tableData" :key="index" style="margin-bottom: 10px;">
            <el-collapse-item style="border-left: 1px solid #EBEEF5;border-right: 1px solid #EBEEF5;">
              <el-row slot="title" class="collapse-title">
                <el-col :md="8" :xs="24">
                  <el-button type="primary" size="medium" style="float: left;width: 58px;height: 40px;">
              ...
              <div class="el-timeline">
                <el-timeline>
                  <el-timeline-item v-for="(item2, index2) in item.details" :key="index2" :timestamp="item2.acceptTime" :type=" index2===0 ? 'primary': 'success'">
...
export default {
  ...
  created() {
    this.countryCacheList = getCountryListCache()
    this.stateBtns = [{
      recState: -1,
      recStateName: '全部',
